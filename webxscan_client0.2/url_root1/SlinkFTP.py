...
class CS_linkftp(threading.Thread):
    def __init__(self,openurl):
        ...
        self.openftp=openurl[1].split('//')[1]
        #self.Chost=""  #主机地址
    ...
    try:
        ...
        print u"找不到文件username.dic  password.dic",e
    ...
    #尝试登录  if success return username & password
        ...
        try:  #判断是否为匿名账户  #anonymous  密码为空属于匿名账户
            ...
            EXP_list=[1,self.openurl[0],host,"FTP","user:anonymous:password:","","niming"]
...
#                            print u_data
                            EXP_list=[1,self.openurl,host,"FTP","user:"+account[0]+"--password:"+account[1]+"","",""]
                            #["01可信度","主网址","从网址","漏洞类型","漏洞地址","密码","备注"] 7个
                            url_exp.put(EXP_list,0.5)   #插入队列
        ...
        for i in range(nthreads):  #nthreads=10  创建10个线程
            threads.append(crackThread(self.openurl[0]))
...
if __name__=='__main__':
    threads = []  #线程
    list=["http://www.baidu.com","http://127.0.0.1"]   #扫描网址   附加数据
    for i in range(1):  #nthreads=10  创建10个线程
        threads.append(CS_linkftp(list))
