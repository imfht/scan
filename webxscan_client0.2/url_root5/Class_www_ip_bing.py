...
class Class_www_ip_bing:  #查找2级域名
...
#            self.penurl=penurl
    ...
    def scan(self,arg):
        ...
        self.NO_url ="msn.com|microsoft.com"
        self.NO_url_list=self.NO_url.split('|')
#        self.bing_ip_www("192.157.211.40")  #192.157.211.40
    ...
    def liet_CX(self):  #查询数据是否存在
        try:
            ...
            while E < len(self.list_2):
                print self.list_2[E]
                list_url=[self.url0,"http://"+self.list_2[E]]
                url_root1.put(list_url,0.3)   #插入队列
                E = E + 1
            return 0
        except:
            pass
    ...
    def bing_ip_www(self,ip):
        try:
            ...
            #re.compile( r"<td class='' ><span class='word-wrap'>(.*?)</span></td>")
...
#        t.start()  #start就是开始线程
    Class_www=Class_www_ip_bing()
    Class_www.scan(Class_www.assign('bingIPwww', ("http://www.baidu.com","http://www.aikanqiu.com"))[1])
