...
#        #===标记扫描过
#        #        sql_up(host,"send")
#        up = "update  url set  ftpsend='send' where url='%s'"%(host)
#        mysql.mysql_update(up)  #修改数据
#        mysql.mysql_S()  #保存数据
#    #        #SQL修改数据
#    except:
#        print u"---FTP21没有开放",
...
#        up = "update  url set  ftpsend='no21' where url='%s'"%(host)
#        mysql.mysql_update(up)  #修改数据
#        mysql.mysql_S()  #保存数据
#        try:
#            ftpB.quit() #退出ftp服务器
...
#                print "-_-"  #添加失败
        ...
        mysql.mysql_S()  #保存数据
        print u"\n-----------------------URL列表添加完成-----------------------"
        print u"-------扫描过URL:%s个/URL读取失败%s个/成功添加到数据库URL:%s个-------"%(INI_data1,INI_data2,INI_data3)
        open_mysql()  #读取URL

        # 上面是将每条<a></a>里面的内容和地址给匹配出来
    except:
        print u"这个URL地址无效！！！！！！！！！！！！！！"
        INI_data2=INI_data2+1  #URL获取失败
        mysql.mysql_S()  #保存数据
        open_mysql()  #读取URL
##################################################
