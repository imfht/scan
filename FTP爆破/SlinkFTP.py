    ...
    def run(self):
        try:
            ...
            if self.openftp.empty():   #判断队列是否为空
                print "Thread:%d--CS_linkftp-data ftp_url  null"%(self.Ht)
                time.sleep(100)
                self.run()
...
#                            print str(e)    #调试信息
                        ...
                        try:
                            ...
                            #没有异常发生，这是一个正确的帐号
                            u_data="Thread:%d-linkftp-FTP====OK====IP:%s  user:%s  password:%s  time:%s"%\
                                   (self.AHt,host,account[0],account[1],time.strftime('%Y.%m.%d-%H.%M.%S'))
                            print u_data
                            cc_data="IP:%s   user:%s   password:%s"%(host,account[0],account[1])
                            self.TXT_file(cc_data)  #写入文本彭立20
        ...
        print "===Thread:%d-CS_linkftp-%s ftp Scan End %s---With time %s S"%(self.Ht,host,time.strftime('%Y.%m.%d-%H.%M.%S'),int(time.strftime('%H%M%S'))-self.A)
        self.run()     #读取URL
