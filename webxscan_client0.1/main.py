...
class oprn_url(threading.Thread):
    ...
    def url_data(self,data):#http://www.126.com|135
        ...
        #函数对结果进行分析  判断扫描等级
            ...
            # url_root5  #C段扫描
            if '1' in ss[1]:
                url_root1.put(ss[0],0.3)   #插入队列
            if '2' in ss[1]:
                url_root2.put(ss[0],0.3)   #插入队列
            if '3' in ss[1]:
                url_root3.put(ss[0],0.3)   #插入队列
            if '4' in ss[1]:
                url_root4.put(ss[0],0.3)   #插入队列
            if '5' in ss[1]:
                url_root5.put(ss[0],0.3)   #插入队列
        except:
            return 0
...
if __name__=='__main__':
    ...
    print "           OS:Windows   VS:0.1"
    print "           time:16/7/2013"
    try:
        threads = []  #线程
        for i in range(1):
