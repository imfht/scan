        ...
        try:
            ...
            if httpres:
                ...
                if code != testcode: #不相等为有效
                    ...
                    #是
                        EXP_list=[1,self.url,"exp","CN_exp_dedecms_getshell",arg.strip()+site,"long","webshell"]
                        #["一句话 是否成功0否 1是","网址","严重程度","漏洞类型","漏洞地址","密码","备注"]7个
                        print EXP_list
                        url_exp.put(EXP_list,0.5)   #插入队列
                    else:
                    #否
                        EXP_list=[0,self.url,"exp","CN_exp_dedecms_getshell",arg.strip()+site,"long","webshell"]
                        #["一句话 是否成功0否 1是","网址","严重程度","漏洞类型","漏洞地址","密码","备注"]7个
                        print EXP_list
                        url_exp.put(EXP_list,0.5)   #插入队列
