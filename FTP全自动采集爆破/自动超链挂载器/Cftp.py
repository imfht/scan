...
class CS_linkftp(threading.Thread):
    ...
    def admin_open_FTP(self,host,user,passwd):#遍历数组
        try:
            ...
            #print u"获取别表完成"
                ...
                self.int_label_1+=self.FTP_list  ##检测到%s个页面
    ...
    def open_label(self):  #更新数据
        print u"连接FTP成功%s个/失败%s个/"%(self.int_label_0_1,self.int_label_0_2),

        print u"检测到%s个页面/"%(self.int_label_1),

        print u"下载成功%s个页面/"%(self.int_label_2),

        print u"超连接已存在%s个页面/"%(self.int_label_3),

        print u"上传成功%s个页面"%(self.int_label_4)
        ###########
    def open_txt(self):  #打开TXT文本写入数组
        try:
            xxx = file('wwwroot.txt', 'r')
