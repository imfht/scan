...
#-*- coding: UTF-8 -*-
#FTP采集爆破
import Copenftp  #采集FTP地址
import Copenurl  #采集URL地址
import ClinkFTP  #FTP爆破
import CpasswordFTP  #FTP权限检查
import Cmysql_delete  #检测更新数据库
#import Ctitle    #更新网站标题
import time
import ctypes   #DLL调用
import thread
import ConfigParser  #INI读取数据
import Cclose_open #结束进程在从新开启进程  解决MYSQL连接错误
import Cthread #根据数据量判断线程

def Crun():  #启动
    thrint=1
    t_h=Cthread.CS_Cthread()
...
#    ClinkFTP_nthreads = int(config.get("DATA","th_ftppassword"))
    ...
    for i in range(Cclose_open_nthreads):  #nthreads=10  创建10个线程
        Cclose_open_threads.append(Cclose_open.CS_linkftp())
...
#    Ctitle_threads = []  #线程
#    Ctitle_nthreads=1
#    for i in range(Ctitle_nthreads):  #nthreads=10  创建10个线程
#        Ctitle_threads.append(Ctitle.CS_title())
#
#    for thread in Ctitle_threads:   #不理解这是什么意思    是结束线程吗
#        thread.start()  #start就是开始线程
...
if __name__=='__main__':
    print u"---------------------------------------------------------------------"
    print u"          欢迎使用FTP--webshell权限查找器1.4-----BY：落雪               "
